...
function checkCorrectDate(dateArray) {
  ...
  const [day, month, year] = dateArray.map(Number);
  ...
  if (month === 2 && (day === 30 || (day === 29 && !isLeapYear(year)))) {
    return false;
  }
...
function checkDateFormat(date) {
  ...
  if (dateNumbersArray === null || dateNumbersArray.some(num => isNaN(num) || num < 0)) {
    return false;
  }

  return checkCorrectDate(dateNumbersArray);
}

function getDates(arr) {
  return arr
    .filter(checkDateFormat)
    .map(date => transformDateStringToArray(date).join('-'));
}
